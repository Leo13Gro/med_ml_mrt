...
class BuildDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transforms:
            img = self.transforms(np.array(img / (255 * 255), dtype=np.float32))
            img = img[None, :, :, :]
...
class SegmentationModel(ModelABC):
    ...
    def __init__(self, model_type: str):  # model_type='all'/'long'/'cross'
        ...
        self.load(path=settings['segmentation']['cross'])
        ...
        self.transforms = {
            "prod":
                T.Compose([
                    T.ToTensor(),
                    T.transforms.CenterCrop(self.target_shape),
    ...
    def load(self, path: list) -> None:
        print(path)
        self._model = smp.UnetPlusPlus(encoder_name="efficientnet-b6", encoder_weights=None, in_channels=1, classes=1)
        self._model.to(self.device)
        self._model.load_state_dict(torch.load(path, map_location=self.device), strict=False)
        self._model.eval()
    ...
    @staticmethod
    def preprocessingShape(img: object) -> object:
        img = Image.fromarray(img).convert(mode='L')
        img_array = np.array(img, dtype=np.float32)
    ...
    def center_crop(self, img: np.ndarray, output_size: tuple):
    ...
    def predict(self, group: list) -> tuple:
        ...
        """
        self.source_shape = self.preprocessingShape(group[0])
        dataset = BuildDataset(group, transforms=self.transforms['prod'])
        ...
        with torch.no_grad():
            for index, im in enumerate(iter(dataset)):
                ...
                with torch.no_grad():
                    ...
                    img_array = (torch.squeeze(im)).data.cpu().numpy()
                    images_features.append(img_array)

                    with torch.no_grad():
                        mask_c1 = self._model(im)
        ...
        for nodules, features in zip(selected_nodules, images_features):
            img_array = features
            cut_image_orshape = self.target_shape
            or_shape = self.source_shape
            location = features[3]
            new_nodules = []
            coordinates = []
            mask_array = np.zeros(shape=(224, 224), dtype=np.float32)
            ...
            for node in nodules:
                ...
                cropped_image = self.center_crop(n1_array, self.target_shape)
                f1_mask = T.Pad(padding=((self.source_shape[0] - self.target_shape[0]) // 2))(
                    torch.tensor(cropped_image))

                # n1_array = resize(n1_array, cut_image_orshape, order=1)
                # f1_mask[location[0]:location[1],
                #         location[2]:location[3]] = n1_array
                f1_mask = (f1_mask > 0.5).numpy()
            ...
            cropped_image = self.center_crop(mask_array, self.target_shape)
            final_mask = T.Pad(padding=((self.source_shape[0] - self.target_shape[0]) // 2))(
                torch.tensor(cropped_image))
