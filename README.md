# Сервис обработки МРТ и КТ снимков
...
4) создаем топики для redpanda (потом автоматизируем)
    + localhost:8081    
    + вкладка topics
    + создаем по 3 топика для КТ и МРТ (processed и splitted обязательно создавать руками или python упадет):
        - mriupload
        - mrisplitted
        - mriprocessed
        - ktupload
...
6) __ОПЦИОНАЛЬНО__ : каждый снимок МРТ должен быть привязанно к device, нужно добавить его в бд чтобы работало
    * pgAdmin/DataGrip/Расширение vscode для баз данных, заходите в examdb, таблица `device` добавляете что угодно, этот id потом юзаете при post /mri/mris
